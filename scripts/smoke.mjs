...
await flow('down payment amount ↔ percent conversion', async () => {
  const page = await open()
  await page.getByRole('tab', { name: /^Installments$/ }).click()
  await page.waitForTimeout(800)
  await fields(page).nth(0).fill('10000000')
  await fields(page).nth(1).fill('2000000')
  await page.getByRole('tab', { name: 'Percent' }).click()
  await page.waitForTimeout(300)
  const asPercent = digits(await fields(page).nth(1).inputValue())
  check('down payment converts amount → percent', asPercent === '20', asPercent)
  await page.getByRole('tab', { name: 'Amount' }).click()
  await page.waitForTimeout(300)
  // The field displays with live thousands grouping (`formatLiveInput`), so compare digits only.
  const asAmount = digits(await fields(page).nth(1).inputValue())
  check('down payment round-trips percent → amount', asAmount === '2000000', asAmount)
  await page.close()
})

await flow('Enter key submits the calculator', async () => {
  const page = await open()
  await fields(page).nth(0).fill('100000')
  const margin = fields(page).nth(1)
  await margin.fill('20')
  await margin.press('Enter')
  const body = await resultText(page)
  check('pressing Enter in a field computes a result', digits(body).includes('120000'))
  await page.close()
})

await flow('stale result clears on edit', async () => {
  const page = await open()
  await fields(page).nth(0).fill('100000')
  await fields(page).nth(1).fill('20')
  await calc(page)
  await resultText(page)
  await fields(page).nth(0).fill('150000')
  await page.waitForTimeout(300)
  const stillShown = await page.locator('section[aria-label="Result"]').count()
  check('editing a field after Calculate clears the stale result', stillShown === 0)
  await page.close()
})

await flow('lens requires the restock price once "known" is chosen', async () => {
  const page = await open()
  await fields(page).nth(0).fill('100000')
  await fields(page).nth(1).fill('20')
  await page.getByRole('radio', { name: '3 mo' }).click()
  await page.waitForTimeout(400)
  await page.getByRole('tab', { name: /restock price/ }).click()
  await page.waitForTimeout(300)
  await calc(page)
  await page.waitForTimeout(400)
  const requiredAlert = await page.getByRole('alert').filter({ hasText: /required/i }).count()
  check('leaving the restock price blank shows a required error', requiredAlert > 0)
  const resultShown = await page.locator('section[aria-label="Result"]').count()
  check('no result renders while the restock price is missing', resultShown === 0)
  await page.close()
})
...
await flow('products', async () => {
  ...
  // A negative price/margin used to save silently and only surface as a nonsense health chip.
  await page.getByText('Blue mug', { exact: true }).click()
  await page.waitForTimeout(700)
  await page.locator('#product-price').fill('-5000')
  await page.locator('[role="dialog"]').getByRole('button', { name: /Save to my products/ }).click()
  await page.waitForTimeout(300)
  const negativePriceBlocked = await page.locator('[role="dialog"]').count()
  check('a negative selling price blocks Save and keeps the sheet open', negativePriceBlocked > 0)
  const priceAlert = await page.locator('[role="dialog"]').getByRole('alert').count()
  check('a negative selling price shows an error', priceAlert > 0)
  await page.keyboard.press('Escape')
  await page.waitForTimeout(400)

  // Selecting the product and repricing by less than -100% used to clamp silently with no error.
  await page.getByRole('button', { name: /^Reprice$/ }).click()
  await page.waitForTimeout(500)
  await page.getByRole('checkbox', { name: 'Blue mug' }).click()
  await page.waitForTimeout(300)
  await page.getByRole('button', { name: /^Reprice$/ }).click()
  await page.waitForTimeout(800)
  await page.locator('[role="dialog"] input[inputmode="decimal"]').last().fill('-150')
  await page.waitForTimeout(300)
  const bulkAlert = await page.locator('[role="dialog"]').getByRole('alert').count()
  check('a below -100% bulk cost change shows an error', bulkAlert > 0)
  const applyDisabled = await page.locator('[role="dialog"]').getByRole('button', { name: /^Apply$/ }).isDisabled()
  check('Apply stays disabled while the bulk percent is invalid', applyDisabled)
  await page.keyboard.press('Escape')
  await page.close()
})
